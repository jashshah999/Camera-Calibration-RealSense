...
cv2.createTrackbar("L-H", "Trackbars", 157, 180, nothing)
cv2.createTrackbar("L-S", "Trackbars", 100, 255, nothing)
cv2.createTrackbar("L-V", "Trackbars", 20, 255, nothing)
cv2.createTrackbar("U-H", "Trackbars", 175, 180, nothing)
    ...
    if len(aruco_corners) > 0:
        ...
        for (markerCorner, markerID) in zip(aruco_corners, ids):
            ...
            for i in range(0, len(centers)-1):
                ...
                x1_arucos = centers[i][0]
                y1_arucos = centers[i][1]
                x2_arucos = centers[i+1][0]
                y2_arucos = centers[i+1][1]

                m_arucos = (y2_arucos - y1_arucos)/(x2_arucos-x1_arucos)
            ...
            for cnt in contours:
                ...
                if area > 1:
                    ...
                    cv2.line(frame, (cX,cY), centers[0],(255,0,0),2 )
                    ...
                    if len(approx) == 4:
                        ...
                        cv2.circle(frame, (approx[0][0]),7,(0, 0, 255),-1) #pointers at each corner (Top Right)
                        cv2.circle(frame, (approx[1][0]),7,(255, 0, 255),-1) #top left
                        cv2.circle(frame, (approx[2][0]),7,(255, 0, 255),-1) #bottom left
                        cv2.circle(frame, (approx[3][0]),7,(255, 0, 255),-1)  #bottom right

                        bottom_right_y = approx[3][0][1]
                        bottom_right_x = approx[3][0][0]
                        bottom_left_y = approx[2][0][1]
                        bottom_left_x = approx[2][0][0]

                        m_marker = (bottom_right_y - bottom_left_y)/(bottom_right_x- bottom_left_x)
        ...
        #---------------------------------------------------------------------------------------------------------------------#
                        ...
                        box = np.int0(box)
                        angle = np.rad2deg(np.arctan2((m_arucos-m_marker),(1+(m_arucos*m_marker))))
                        # print("angle:")
                        # print(m_marker - m_arucos)

                        # label = "  Yaw: " + str(np.abs(m_marker - m_arucos)) + " degrees"
                        # textbox = cv2.rectangle(frame, (center[0]-35, center[1]-25), 
                        #     (center[0] + 295, center[1] + 10), (255,255,255), -1)
                        # cv2.putText(frame, label, (center[0]-100, center[1]), 
                        #     cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,0,0), 1, cv2.LINE_AA)
                        cv2.putText(frame, "Yaw: "+ str(angle), (100,100),font,0.7,(0,0,0),1,cv2.LINE_AA)
